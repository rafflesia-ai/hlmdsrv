...
func (a app) docsCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "docs",
		Short: "Generate MDsrv headless CLI reference documentation",
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) != 0 {
				return fmt.Errorf("docs does not accept positional arguments")
			}
			...
			fmt.Fprintf(a.stdout, "wrote MDsrv CLI docs to %s\n", target)
			return nil
		},
	}
